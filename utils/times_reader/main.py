...
def output_raw_tables_from_xl(
    ...
    if use_pkl and os.path.isfile(pickle_file):
        ...
        input_files = [
            str(path)
            for path in Path(input_dir).rglob("*.xlsx")
            if not path.name.startswith("~")
        ]
        print(f"Loading {len(input_files)} files from {input_dir}")
        raw_tables = []
    ...
    dump_tables_modified(
        raw_tables,
        os.path.join(output_dir, "raw_tables.txt"),
        input_dir
    )
    return
